...
public class Generator {
    ...
    void readContracts() throws Exception {
        try (var reader = new FileReader("_generator/contracts.csv")) {
    ...
    void readGames() throws Exception {
        try (var reader = new FileReader("_generator/games.csv")) {
            var records = CSVFormat.Builder.create().setHeader().build().parse(reader);
            for (var record : records) {
                var name = record.get("Name");
                var slug = record.get("Slug");
                ...
                var tags = Arrays.stream(record.get("Tags").split(";")).toList();
                games.add(new Game(name, slug, url, status, tags, contracts.get(slug)));
            }
        }
    ...
    void readPodcasts() throws Exception {
        try (var reader = new FileReader("_generator/podcasts.csv")) {
            var records = CSVFormat.Builder.create().setHeader().build().parse(reader);
            for (var record : records) {
                var name = record.get("Name");
                var url = record.get("URL");
    ...
    void readBlogs() throws Exception {
        try (var reader = new FileReader("_generator/blogs.csv")) {
            var records = CSVFormat.Builder.create().setHeader().build().parse(reader);
            for (var record : records) {
                var name = record.get("Name");
                var url = record.get("URL");
